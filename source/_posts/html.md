...
# html属性
## 全局属性
1. accesskey 可以给元素添加快捷键,当你使用alt+n后会自动聚焦，或激活该元素，和浏览器快捷键有冲突时，优先级低于浏览器快捷键
```html
  <input accesskey="n"  value="自动聚焦">
  <button accesskey="s">激动激活</button>
```
2. draggable dropzone
4. hidden
5. lang 表示元素的使用语言
```html
...
标准的事件处理模型分为三个阶段：
  ...
  事件流有三个阶段：捕获 → 目标 → 冒泡。 （阶段是三个 但是触发模式要么是捕获模式 要么是冒泡模式， 默认是冒泡）
  默认情况（addEventListener 的第三个参数为 false 或不传）：监听器只在冒泡阶段触发。
  要监听捕获阶段：必须将第三个参数设为 true（或 { capture: true }）
